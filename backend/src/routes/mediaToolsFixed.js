import express from "express";
import multer from "multer";
import fs from "fs";
import path from "path";
import crypto from "crypto";
import { execFile } from "child_process";
import { promisify } from "util";
import { optionalApiKey } from "../middleware/optionalApiKey.js";

const router = express.Router();
const execFileAsync = promisify(execFile);
const tempDirectory = path.resolve("temp");
if (!fs.existsSync(tempDirectory)) fs.mkdirSync(tempDirectory, { recursive: true });

const storage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, tempDirectory),
  filename: (_req, file, cb) => cb(null, `${crypto.randomUUID()}${path.extname(file.originalname)}`),
});

// Video files are commonly much larger than images. Keep the same limit for every
// tool, but raise it to 500 MB so normal phone/camera videos are not rejected.
const upload = multer({ storage, limits: { fileSize: 500 * 1024 * 1024 } });

function cleanup(...files) {
  files.flat().forEach((file) => {
    if (file && fs.existsSync(file)) {
      try { fs.unlinkSync(file); } catch {}
    }
  });
}

function safeName(name) {
  return String(name || "download")
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, "_")
    .replace(/\.\.+/g, "_")
    .slice(0, 180) || "download";
}

function toolError(error, fallback) {
  const stderr = String(error?.stderr || "").trim();
  const message = String(error?.message || "").trim();
  if (/ENOENT/i.test(message)) {
    return "This media processor is not installed on the ConvertFlow server. Please check the Render media dependencies.";
  }
  return stderr || message || fallback;
}

async function sendFile(res, input, output, downloadName) {
  if (!fs.existsSync(output)) throw new Error("The processing tool did not create an output file.");
  res.download(output, safeName(downloadName), (error) => {
    cleanup(input, output);
    if (error) console.error("Media tool download error:", error);
  });
}

function requireFile(req, message) {
  if (!req.file) {
    const error = new Error(message);
    error.status = 400;
    throw error;
  }
}

// The original image routes were using ImageMagick commands that can fail on
// particular image metadata/fonts. These versions keep the same API but use
// conservative arguments and validate every output.
router.post("/image-crop", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No image was uploaded.");
    const width = Math.max(1, Math.min(10000, Number(req.body.width) || 800));
    const height = Math.max(1, Math.min(10000, Number(req.body.height) || 600));
    const x = Math.max(0, Number(req.body.x) || 0);
    const y = Math.max(0, Number(req.body.y) || 0);
    output = path.join(tempDirectory, `${crypto.randomUUID()}-cropped.png`);
    await execFileAsync("magick", [
      req.file.path,
      "-auto-orient",
      "-crop", `${width}x${height}+${x}+${y}`,
      "+repage",
      "-strip",
      output,
    ], { timeout: 120000 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-cropped.png`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Image crop error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Image crop failed.") });
  }
});

router.post("/image-watermark", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No image was uploaded.");
    const text = String(req.body.text || "ConvertFlow").replace(/[\r\n]/g, " ").slice(0, 100);
    output = path.join(tempDirectory, `${crypto.randomUUID()}-watermarked.png`);
    await execFileAsync("magick", [
      req.file.path,
      "-auto-orient",
      "-gravity", "southeast",
      "-fill", "white",
      "-stroke", "black",
      "-strokewidth", "1",
      "-pointsize", "36",
      "-annotate", "+30+30", text,
      "-strip",
      output,
    ], { timeout: 120000 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-watermarked.png`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Image watermark error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Watermark failed.") });
  }
});

router.post("/image-enhance", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No image was uploaded.");
    output = path.join(tempDirectory, `${crypto.randomUUID()}-enhanced.png`);
    await execFileAsync("magick", [
      req.file.path,
      "-auto-orient",
      "-colorspace", "sRGB",
      "-contrast-stretch", "0x1%",
      "-unsharp", "0x0.8+0.8+0.02",
      "-strip",
      output,
    ], { timeout: 120000 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-enhanced.png`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Image enhance error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Image enhancement failed.") });
  }
});

router.post("/image-background-remover", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No image was uploaded.");
    output = path.join(tempDirectory, `${crypto.randomUUID()}-background-removed.png`);
    await execFileAsync("magick", [
      req.file.path,
      "-auto-orient",
      "-alpha", "on",
      "-fuzz", "10%",
      "-fill", "none",
      "-draw", "matte 0,0 floodfill",
      "-strip",
      output,
    ], { timeout: 120000 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-no-background.png`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Background remover error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Background removal failed.") });
  }
});

function videoArgsBase(output) {
  return ["-hide_banner", "-loglevel", "error", "-y", output];
}

router.post("/video-compress", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No video was uploaded.");
    const quality = String(req.body.quality || "medium").toLowerCase();
    const crf = quality === "low" ? "32" : quality === "high" ? "23" : "28";
    const resolution = String(req.body.resolution || "original");
    output = path.join(tempDirectory, `${crypto.randomUUID()}-compressed.mp4`);
    const args = ["-hide_banner", "-loglevel", "error", "-i", req.file.path, "-c:v", "libx264", "-crf", crf, "-preset", "veryfast", "-c:a", "aac", "-b:a", "128k", "-movflags", "+faststart"];
    if (["1080p", "720p", "480p"].includes(resolution)) args.push("-vf", `scale=-2:${resolution.replace("p", "")}`);
    args.push(...videoArgsBase(output));
    await execFileAsync("ffmpeg", args, { timeout: 300000, maxBuffer: 1024 * 1024 * 4 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-compressed.mp4`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Video compression error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Video compression failed.") });
  }
});

router.post("/video-resolution", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No video was uploaded.");
    const resolution = String(req.body.resolution || "1080p").toLowerCase();
    const height = { "4k": 2160, "1440p": 1440, "1080p": 1080, "720p": 720, "480p": 480 }[resolution] || 1080;
    output = path.join(tempDirectory, `${crypto.randomUUID()}-resolution.mp4`);
    await execFileAsync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-i", req.file.path, "-vf", `scale=-2:${height}:flags=lanczos`, "-c:v", "libx264", "-preset", "veryfast", "-c:a", "aac", "-movflags", "+faststart", ...videoArgsBase(output)], { timeout: 300000, maxBuffer: 1024 * 1024 * 4 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-${resolution}.mp4`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Video resolution error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Video resolution conversion failed.") });
  }
});

router.post("/video-framerate", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No video was uploaded.");
    const fps = Math.max(1, Math.min(120, Number(req.body.fps) || 30));
    output = path.join(tempDirectory, `${crypto.randomUUID()}-fps.mp4`);
    await execFileAsync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-i", req.file.path, "-r", String(fps), "-c:v", "libx264", "-preset", "veryfast", "-c:a", "aac", "-movflags", "+faststart", ...videoArgsBase(output)], { timeout: 300000, maxBuffer: 1024 * 1024 * 4 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-${fps}fps.mp4`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Video framerate error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Frame-rate conversion failed.") });
  }
});

router.post("/video-mute", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No video was uploaded.");
    output = path.join(tempDirectory, `${crypto.randomUUID()}-muted.mp4`);
    await execFileAsync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-i", req.file.path, "-c:v", "copy", "-an", ...videoArgsBase(output)], { timeout: 300000, maxBuffer: 1024 * 1024 * 4 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-muted.mp4`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Video mute error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Removing video audio failed.") });
  }
});

router.post("/video-speed", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No video was uploaded.");
    const speed = Math.max(0.25, Math.min(4, Number(req.body.speed) || 1));
    output = path.join(tempDirectory, `${crypto.randomUUID()}-speed.mp4`);
    const pts = (1 / speed).toFixed(4);
    const audioFilters = [];
    let remaining = speed;
    while (remaining > 2) { audioFilters.push("atempo=2"); remaining /= 2; }
    while (remaining < 0.5) { audioFilters.push("atempo=0.5"); remaining /= 0.5; }
    audioFilters.push(`atempo=${remaining.toFixed(4)}`);
    await execFileAsync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-i", req.file.path, "-filter_complex", `[0:v]setpts=${pts}*PTS[v];[0:a]${audioFilters.join(",")}[a]`, "-map", "[v]", "-map", "[a]?", "-c:v", "libx264", "-preset", "veryfast", "-c:a", "aac", ...videoArgsBase(output)], { timeout: 300000, maxBuffer: 1024 * 1024 * 4 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-${speed}x.mp4`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Video speed error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Video speed change failed.") });
  }
});

router.post("/video-thumbnail", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No video was uploaded.");
    const seconds = Math.max(0, Number(req.body.time) || 1);
    output = path.join(tempDirectory, `${crypto.randomUUID()}-thumbnail.jpg`);
    await execFileAsync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-ss", String(seconds), "-i", req.file.path, "-frames:v", "1", "-q:v", "2", ...videoArgsBase(output)], { timeout: 120000, maxBuffer: 1024 * 1024 * 4 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}-thumbnail.jpg`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("Video thumbnail error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "Thumbnail extraction failed.") });
  }
});

router.post("/gif", optionalApiKey, upload.single("file"), async (req, res) => {
  let output;
  try {
    requireFile(req, "No video was uploaded.");
    const fps = Math.max(1, Math.min(30, Number(req.body.fps) || 12));
    output = path.join(tempDirectory, `${crypto.randomUUID()}.gif`);
    await execFileAsync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-i", req.file.path, "-vf", `fps=${fps},scale=720:-1:flags=lanczos`, "-loop", "0", ...videoArgsBase(output)], { timeout: 300000, maxBuffer: 1024 * 1024 * 4 });
    return sendFile(res, req.file.path, output, `${path.parse(req.file.originalname).name}.gif`);
  } catch (error) {
    cleanup(req.file?.path, output);
    console.error("GIF creation error:", error);
    return res.status(error.status || 500).json({ success: false, message: toolError(error, "GIF creation failed.") });
  }
});

export default router;
