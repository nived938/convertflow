...
async function sendAuthEmail({ type, email, code }) {
  ...
  if (!webhookUrl) {
    throw new Error("N8N_LOGIN_WEBHOOK_URL is not configured.");
  }
  ...
  if (!response.ok) {
    ...
    throw new Error("Authentication email service is unavailable.");
  }
...
export async function verifyEmail(req, res) {
  try {
    ...
    if (!user || user.verification_code !== String(code || "")) {
      return res.status(400).json({
        success: false,
  ...
  try {
    ...
    if (!user.email_verified) {
      ...
      return res.status(200).json({
        ...
        message: "Check your email for the login verification code."
      });
    }
