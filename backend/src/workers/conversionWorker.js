...
export function getJobInternal(
  jobId
) {
...
async function processJob(
  ...
  for (
    ...
    try {
      console.log(
        `Job ${job.id}: ${file.originalName} → ${file.outputFormat}`
      );
      ...
      console.error(
        `File conversion failed: ${file.originalName}`,
        error
      );
  ...
  try {
    ...
    console.error(
      `ZIP creation failed for job ${job.id}:`,
      error
    );
  }
}
...
function isFileUsedByActiveJob(
  filePath
) {
  for (
  ...
  for (const filename of files) {
    ...
    try {
      ...
      if (
        ...
        console.log(
          `Removed old temporary file: ${filename}`
        );
      }
    } catch (error) {
      console.error(
        `Could not clean ${filename}:`,
        error
      );
    }
  }
