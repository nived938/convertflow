...
export default function Header() {
  ...
  const [light, setLight] = useState(() => localStorage.getItem("convertflow_theme") === "light");
  useEffect(() => { document.documentElement.dataset.theme = light ? "light" : "dark"; localStorage.setItem("convertflow_theme", light ? "light" : "dark"); }, [light]);
  ...
  return <header className="site-header"><div className="header-inner">
    ...
    <nav className={`header-nav ${mobileOpen ? "mobile-open" : ""}`}>
      {link("/", "Home")}{link("/convert", "Convert")}{link("/faq", "FAQ")}{link("/about", "About")}{link("/api", "API")}{link("/formats", "Formats")}{link("/history", "History")}{link("/status", "Status")}
      <button className="theme-toggle" type="button" onClick={() => setLight((v) => !v)} aria-label={light ? "Use dark mode" : "Use light mode"}>{light ? <Moon size={16} /> : <Sun size={16} />}</button>
