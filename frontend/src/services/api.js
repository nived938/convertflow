...
async function refreshBackendAccess() {
  if (Date.now() - controlLoadedAt < 10000) return backendEnabled;
  try {
    const response = await fetch("/api/site-mode", { cache: "no-store" });
    const data = await response.json().catch(() => ({}));
    if (data.success) { backendEnabled = data.backendEnabled !== false; controlLoadedAt = Date.now(); }
  } catch {}
  return backendEnabled;
}
async function ensureBackendAccess() {
  if (!(await refreshBackendAccess())) throw new Error("ConvertFlow backend access is currently disabled by the administrator.");
}
function authHeaders(headers = {}) {
  const token = localStorage.getItem("convertflow_token"); const apiKey = localStorage.getItem("convertflow_api_key");
  return { ...headers, ...(token ? { Authorization: `Bearer ${token}` } : {}), ...(apiKey ? { "X-ConvertFlow-API-Key": apiKey } : {}) };
}

export async function checkBackendHealth() {
  await ensureBackendAccess(); const response = await fetch(`${API_BASE_URL}/health`, { credentials: "include" }); const text = await response.text(); let data = {};
  if (text.trim()) { try { data = JSON.parse(text); } catch { data = { message: text }; } }
  if (!response.ok) throw new Error(data.message || `Backend responded with HTTP ${response.status}`); return data;
}

function upload(path, formData, onProgress, fallback) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest(); xhr.open("POST", `${API_BASE_URL}${path}`); xhr.withCredentials = true; xhr.responseType = "blob";
    ...
    xhr.onload = () => xhr.status >= 200 && xhr.status < 300 ? resolve(xhr.response) : reject(new Error(fallback));
...
export async function convertFile(file, outputFormat, onProgress) { await ensureBackendAccess(); const formData = new FormData(); formData.append("file", file); formData.append("outputFormat", outputFormat); return upload("/conversion/convert", formData, onProgress, "The server could not convert this file."); }
export async function convertBatch(files, onProgress) { await ensureBackendAccess(); const formData = new FormData(); const outputFormats = []; for (const item of files) { formData.append("files", item.file); outputFormats.push(item.outputFormat); } formData.append("outputFormats", JSON.stringify(outputFormats)); return upload("/conversion/convert-batch", formData, onProgress, "Batch conversion failed."); }
export async function createConversionJob(files, onProgress) { return convertBatch(files, onProgress); }

async function json(path, options = {}) {
  await ensureBackendAccess(); const headers = authHeaders(options.headers || {}); const response = await fetch(`${API_BASE_URL}${path}`, { credentials: "include", ...options, headers }); const text = await response.text(); let data = {};
  if (text.trim()) { try { data = JSON.parse(text); } catch { data = { message: text }; } }
  if (!response.ok) throw new Error(data.message || `Request failed with HTTP ${response.status}.`); return data;
}
