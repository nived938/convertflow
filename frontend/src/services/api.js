...
export async function logout() { return json("/auth/logout", { method: "POST" }); }
export async function getCurrentUser() { try { return (await json("/auth/me")).user; } catch { return null; } }
export async function verifyLoginCode(email, code) { return json("/auth/verify-login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, code }) }); }
...
async function blob(path, fallback) { const response = await fetch(`${API_BASE_URL}${path}`, { credentials: "include" }); if (!response.ok) throw new Error(fallback); return response.blob(); }
